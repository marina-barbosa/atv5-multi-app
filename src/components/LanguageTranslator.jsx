...
const LanguageTranslator = () => {
  ...
  const [error, setError] = useState(''); // Define o estado para mensagens de erro

  // Função para traduzir o texto
  const translateText = async () => {
    setError(''); // Limpa a mensagem de erro antes de iniciar a tradução
    try {
      if (!text) {
      ...
      if (response.data.responseData) {
        ...
        setError('Nenhum resultado de tradução foi retornado.');
      }
    } catch (error) {
      setError('Ocorreu um erro ao traduzir o texto. Por favor, tente novamente mais tarde.');
  ...
  return (
    <Container>
      ...
      {error && ( // Exibe a mensagem de erro se houver um erro
        <ErrorContainer>
          <p>{error}</p>
        </ErrorContainer>
      )}
